...
const BookAppointment = () => {
  ...
  // Download Receipt as a PDF document generated from the receipt canvas
  const downloadReceipt = () => {
    ...
    // Convert the canvas to a PDF and download it
    ...
    const finalImageHeight = Math.min(imageHeight, pageHeight - margin * 2);
    const finalImageWidth = (canvas.width * finalImageHeight) / canvas.height;
    const x = (pageWidth - finalImageWidth) / 2;
    const y = margin;

    pdf.addImage(dataUrl, 'PNG', x, y, finalImageWidth, finalImageHeight);
    pdf.save(`Receipt_${confirmedAppointment.token.replace('#', '')}_${confirmedAppointment.date}.pdf`);
  };
